...
class Page extends React.Component {
  ...
  config_json() {
    ...
    const tag_data = {
      1336: {
        base_campaign_id: "441",
        email_campaign_id: "443",
        basket_timeout: "1800"
      },
      pixel_src: "//platform3.cloud-iq.com.au/cartrecovery/",
      domain: "thebbqstore.com.au",
      multi_bytes: "false"
    };
    ...
    const allPages = {
      0: {
        ...
        inputs: {
          match: {
            element_match: '.addtocart',
            element_exclude: '#productsTemp-gallery',
          },
      ...
      1: {
        ...
        inputs: {
          match: {
            element_match: '#productsTemp-gallery',
          },
          add_to_basket: '.addtocart',
        }
      },
      ...
      4: {
        ...
        inputs: {
          match: {
            page_match: '/_mycart',
            page_exclude: 'fn=payment'
          },
          basket_container: '.cartTable tbody tr',
          basket: {
            cloudiqProductImage: 'img',
            cloudiqProductName: 'a h4',
            cloudiqProductQuantity: '[id^=qty]',
            cloudiqProductTotal: '.text-right h4',
          },
          cloudiqBasketTotal: '.success h3',
          cloudiqBasketSubTotal: 'tr:first-child .text-right:last-child',
          paypalExpressShortcut: '.fa-coloured-paypal',
          cloudiqDiscountCode: '[name=addcpn]'
        }
      },
      5: {
        name: 'MiniBasket',
        ...
        inputs: {
          match: {
            element_match: '#neto-dropdown #cartcontents li .left img,.wk_before-checkout',
            page_exclude: '/_mycart,fn=payment'
          },
      ...
      7: {
        ...
        inputs: {
          match: {
            page_match: 'fn=payment',
            element_exclude: '.wk_before-checkout'
          },
          click: '#bt_submit',
          fields: {
            cloudiqConsumerEmail: '#bill_email',
            cloudiqConsumerFirstName: '#bill_first_name',
            cloudiqConsumerLastName: '#bill_last_name',
            cloudiqConsumerCountyState: '#bill_state_sel',
            cloudiqConsumerNumber: '#bill_phone',
            cloudiqConsumerZipCode: '#bill_zip',
          },
          paypalExpressShortcut: '#paypal img',
          cloudiqBasketTotal: '#rcpt_order_total',
          cloudiqDiscountCode: '#addcpn',
          cloudiqBasketSubTotal: 'tr:nth-child(6) td:last-child strong',
          cloudiqBasketPayment: '[id^=method]'
        }
      },
      8: {
          ...
          .map((key) => { //configures the inputs that don't need a transmission group and need a specific behaviour
            switch (key) {
              case 'cloudiqBasketPayment':
                ...
                this.concatTarget(ddl_mapping, targetList, stringName, starter.payment_method, currentInputs, pageMatch, key);
              ...
              case 'basket':
                ...
                currentPage.name.match(/Mini/) ? (delete targetList[target_label].page_load, delete targetList[target_label].params) : null;
        ...
        Object.keys(currentInputs)
          ...
          .map((key) => {
            ...
            currentPage.name.match(/checkout/i) ?
              this.concatTarget(ddl_mapping, targetList, target_label, starter.checkout_field_capture, pageMatch, null, null):
              this.concatTarget(ddl_mapping, targetList, target_label, starter.field_capture, pageMatch, null, null);
      ...
      .map((key) => {
        return Object.keys(targetList[key])
          ...
          .map((field) => { //if it has a match field append the correspondent inverted page match]
            return invertedPageMatch[field] ? (targetList[key][field] = targetList[key][field] + ',' + this.stringify_array(invertedPageMatch[field]),
  ...
  getDefaultPageMatch(targetList) {
    let defaultMatch = this.matchObject; //matchObject has the structure but not the content
    Object.keys(targetList)
      .filter(target => !targetList[target].type && !target.match(/MiniBasket/i))
      .map((target) => {
