...
class Page extends React.Component {
  ...
  config_json() {
    ...
    const tag_data = {
      1497: {
        base_campaign_id: "525",
        email_campaign_id: "527",
        basket_timeout: "1800"
      },
      pixel_src: "//platform3.cloud-iq.com.au/cartrecovery/",
      domain: "candlemaking-au.com",
      multi_bytes: "false"
    };
    ...
    const allPages = {
      ...
      1: {
        ...
        inputs: {
          match: {
            element_match: '.ProductMain',
          },
          add_to_basket: '.AddCartButton',
        }
      },
      2: {
        name: 'Login',
        ...
        inputs: {
          ...
          fields: {
            cloudiqConsumerEmail: '#login_email'
          }
        }
      },
      3: {
        ...
        inputs: {
          click: '.btn.alt',
          match: {
            page_match: 'action=create_account',
          },
          fields: {
            cloudiqConsumerEmail: '#FormField_1',
            cloudiqConsumerFirstName: '#FormField_4',
            cloudiqConsumerLastName: '#FormField_5',
            cloudiqConsumerNumber: '#FormField_7',
            cloudiqConsumerZipCode: '#FormField_13',
          }
        }
      },
      4: {
        ...
        inputs: {
          ...
          basket: {
            container: '.CartContents.Stylize.General > tbody > tr',
    ...
    Object.keys(allPages).map((label) => {
      ...
      if (currentPage.status === 'enabled') { //if page enabled creates the page_load event by default
        ...
        Object.keys(currentInputs).map((key) => {  // creates the logic of the fields
          if (currentInputs[key] && key === 'match') this.buildEmptyMatchObject(currentInputs[key]);
          else if (currentInputs[key] && key !== 'fields' && key !== 'click' && key !== 'match') { //configures the inputs that don't need a transmission group and need a specific behaviour
            //console.log(currentInputs[key]);
            switch (key) {
              ...
              case 'cloudiqOrderNumber':
               this.targetMapping(targetList[target_label], starter.order_number, currentPage, null, key);
                ...
                break;
              case 'basket':
                this.targetMapping(targetList[target_label], starter.basket_page, currentPage, null, 'cloudiq');
                currentPage.name.match(/Mini/) ? delete targetList[target_label].page_load : null;
                targetList[target_label].basket = currentInputs.basket;
                /*create the satellite target*/
          ...
          } else if (key === 'fields' || key === 'click') { // if blur or click -> need to create a copy of the target
  ...
  buildEmptyMatchObject(object) {
    Object.keys(object).map((field) => {
      if (Object.keys(this.matchObject).indexOf(field) === -1)
        this.matchObject[field] = [];
    });
  }
  ...
  excludeConfirmationPage(targetList) {
    ...
    if (key !== 'default_page_load' && key !== 'ConfirmationPage' && key !== 'order_number_capture') {
      Object.keys(targetList[key]).map((field) => {
        if (this.isMatchField(field)) {
          if (invertedPageMatch[field]) { //if it has a match field append the correspondent inverted page match
  ...
  assignObjectIfAbsent(invertedPageMatch, targetList, key) {
   Object.keys(invertedPageMatch).map((matchValue) => {
     if (Object.keys(targetList[key]).indexOf(matchValue) === -1) {
       //console.log(invertedPageMatch[matchValue]);
       targetList[key][matchValue] = this.stringify_array(invertedPageMatch[matchValue]);
     }
   });
   return targetList;
  }
  ...
  getPageMatchObject(ConfirmationPage) {
    ...
    ConfirmationPage ?
      Object.keys(ConfirmationPage).map((key) => {
        if (this.isMatchField(key)) {
  ...
  invertPageMatch(pageMatch) {
    ...
    Object.keys(pageMatch).map((key) => {
      invertedObject = this.appendToInvertedObject(key, pageMatch, invertedObject);
    });
  ...
  getDefaultPageMatch(targetList) {
    ...
    Object.keys(targetList).map((target) => {
      Object.keys(targetList[target]).map((key) => {
        if (Object.keys(defaultMatch).indexOf(key) !== -1) {
          defaultMatch = this.appendToInvertedObject(key, targetList[target], defaultMatch);
        }
      });
    });
    return (defaultMatch);
  ...
  targetMapping(target, map1, map2, map3, includeOnly) {
    if (map1) Object.keys(map1).map((input) => {
      target[input] = (typeof (input) === 'object' ? this.stringify_array(map1[input]) : map1[input]);
    });
    ...
    if (map3) Object.keys(map3).map((input) => {
      target[input] = (typeof (input) === 'object' ? this.stringify_array(map3[input]) : map3[input]);
    });
    return target;
  }
  ...
  render() {
    return ( <div > {this.config_json()} </div>
    )
  }
}
